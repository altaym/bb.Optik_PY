...
# https://raw.githubusercontent.com/Jojojoppe/qandaomr/master/qandaomr/parser.py
...
def prepare_image_create(f):
    ...
    ret, img_th = cv.threshold(img_gray, 127, 255, 0)
    i_cont, i_hier = cv.findContours(img_th, cv.RETR_TREE, cv.CHAIN_APPROX_SIMPLE)
            ...
            if width*height*(500/2180952)<area<width*height*(10000/2180952):
                ...
                if 1-SQUARE_TOLERANCE<approx_area/area<SQUARE_TOLERANCE+1 and has_square_parent(i_hier[0], square_indices, i) is False:
                    ...
                    if count_children(i_hier[0], i) < 2:
                        black = j
                        #cv.drawContours(img, [c], 0, (255, 0, 0), 8)
                    #else:
                        #cv.drawContours(img, [c], 0, (0, 0, 255), 8)
    ...
    #fontColor              = (0,255,0)
    #lineType               = 10
    #cv.putText(img,'1', (sq1[0], sq1[1]), font, fontScale, fontColor, lineType)
    #cv.putText(img,'2', (sq2[0], sq2[1]), font, fontScale, fontColor, lineType)
    #cv.putText(img,'3', (sq3[0], sq3[1]), font, fontScale, fontColor, lineType)
    #cv.putText(img,'4', (sq4[0], sq4[1]), font, fontScale, fontColor, lineType)
    ...
    img = cv.warpPerspective(img, transform, (width, height))
    img = img[0:h, 0:w]
    img_gray = cv.warpPerspective(img_gray, transform, (width, height))
        ...
        if len(approx)==4:
            if width*height*(500/2180952)<area<width*height*(10000/2180952):
                ...
                if 1-SQUARE_TOLERANCE<approx_area/area<SQUARE_TOLERANCE+1 and has_square_parent(i_hier[0], square_indices, i) is False:
                    ...
                    cv.drawContours(img, [c], 0, (0, 255, 0), 4)
        i += 1

    if debug:
        cv.imwrite('conrners.jpg', img)
